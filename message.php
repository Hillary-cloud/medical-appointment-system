...
<?php
    ...
    if(isset($_GET['cancel']))
    {
      ...
      if($query)
      {
        echo "<script>alert('This appointment has been declined');</script>";
      }
    }
?>
<?php 
...
<div class="container-fluid">
    ...
    <div class="card">
    <div class="card-body">
    <h3 class="text-center">Send Message to Patient</h3><br>

    <?php 
if (isset($_POST['send'])) {?>
  <h2 class="text-success">Declined Message Notification Sent Successfully.</h2><br>
<?php }
?>
      ...
      while ($row = mysqli_fetch_array($query)) {
          ?>
                <div class="col-md-4"><label>Patient's Firstname:</label></div>
                  ...
                  <div class="col-md-6"><textarea class="d-none" type="text" name="message"><?php echo "Dear ".ucfirst($row['firstname'])." ".ucfirst($row['lastname']). ", we are sorry to inform you that your appointment with "
                  .$row['doctorName']. ", schedulled to hold on " .$row['appdate']. " at " .$row['apptime']. " has been declined."; ?></textarea></div>

                  <div class="col-md-4"><label></label></div>
                  <div class="col-md-6">
                    <input type="submit" name="send" value="Click to send declined message notification" class="btn btn-primary" id="inputbtn">
                  </div>                  
                </div>
              </form>
