...
if (isset($_POST['updateSpec'])) {
    ...
    $id = $_POST['id'];
    $query = "update spec set specialization='$specialization', doctorFee='$doctorFee' where id = '".$_POST['id']."'";
    $result = mysqli_query($con,$query) or die(mysqli_error($con));
    if ($result) {?>
        <h4 class="text-success">Specialization updated successfully.</h4>
   <?php }else {?>
        <h4 class="text-danger">Specialization could not be updated.</h4>
  <?php }
 }
?>
...
<div class="container-fluid">
    ...
    <form class="form-group" method="post" action="">
              ...
              if(isset($_POST['edit']))
    {
      $query=mysqli_query($con,"select * from spec where id = '".$_POST['id']."'");
      while ($row = mysqli_fetch_array($query)) {
          ...
          ?>    <input type="text" class="d-none" name="id" value="<?php echo $row['id']?>">
                <div class="col-md-4"><label>Specialization:</label></div>
                  ...
                  <div class="col-md-6">
                    <input type="submit" name="updateSpec" value="Click to Update" class="btn btn-primary" id="inputbtn">
                    <a href="specialization.php">Click to go back</a>
                  </div>                  
                </div>
              </form>
              <?php }} ?>
